...
// Purpose: 
//
// $Header: /cvs/myengine/src_main/public/valve/utlrbtree.h,v 1.1 2008/02/29 00:05:49 easap\albertyang Exp $
// $NoKeywords: $
//=============================================================================
...
{
	Assert((i != InvalidIndex()) && (i < m_TotalElements));
...
inline typename CUtlRBTree<T, I>::Links_t const &CUtlRBTree<T, I>::Links( I i ) const 
{
	...
	// modify by lk 2012-02-01 这里的代码存在不安全性，应该加入元素上限的判断条件；即使被人误用也不会导致程序异常，甚至崩溃。
	return ((i != InvalidIndex()) && (i < m_TotalElements)) ? *(Links_t*)&m_Elements[i] :
								 *(Links_t*)&s_Sentinel; 
}
...
{
	// note by lk 2012-02-01 新增的元素对象索引位置
	I newElem;
	
	// Nothing in the free list; add.

	// note by lk 2012-02-01 只有当元素被删除的时候,m_FirstFree才会得到初始值
	if (m_FirstFree == InvalidIndex())
	{
		// note by lk 2012-02-01 如果分配的元素位置全部被占用，自动开辟新的内存块已容纳新的元素
		if (m_Elements.NumAllocated() == m_TotalElements)
			// note by lk 2012-02-01 这里默认分配一个元素的内存十分影响效率，有时间这里要要适当的修改一下
			m_Elements.Grow();
		// note by lk 2012-02-01 设置新元素的索引位置
...
#endif
	// note by lk 2012-02-01 Element函数存在安全隐患，有时间可能需要重写这个类的大部分函数
	// 构造函数，分配一块内存
	Construct( &Element(newElem) );
	// note by lk 2012-02-01 重置链表内存首地址
	ResetDbgInfo();
...
{
	Assert( IsValidIndex(i) && (i != InvalidIndex()) && (i < m_TotalElements));
	Destruct( &Element(i) );
	SetLeftChild( i, m_FirstFree ); // indicates it's in not in the tree
...
{
	// note by lk 2012-02-01
	// 获得节点右子树索引
	if ((elem != InvalidIndex()) && (elem < m_TotalElements))
	{
		I rightchild = RightChild(elem);
		// 设置elem节点的右子树为rightchild节点的左子树
		SetRightChild(elem, LeftChild(rightchild));
		if ((LeftChild(rightchild) != InvalidIndex()) && (LeftChild(rightchild) < m_TotalElements))
...
void CUtlRBTree<T, I>::RotateRight(I elem) 
{
	...
	if ((RightChild(leftchild) != InvalidIndex()) && (RightChild(leftchild) < m_TotalElements))
...
{
	// note by lk 2012-02-01 elem不能为根元素，并且是右子树才会执行如下代码片段
	while (elem != m_Root && IsBlack(elem)) 
	{
		...
		if (elem == LeftChild(parent)) 
		{
            ...
            {
				if ((sibling != InvalidIndex()) && (sibling < m_TotalElements))
...
void CUtlRBTree<T, I>::Unlink( I elem )
{
	if ((elem != InvalidIndex()) && (elem < m_TotalElements))
	{
		...
		// note by lk 2012-02-01 如果x的值为有效值,就重新设置该节点的父节点，父节点为要删除元素的父节点
		if ((x != InvalidIndex()) && (x < m_TotalElements))
			SetParent( x, Parent(y) );
		// not by lk 2012-02-01 如果被删除的元素不是根节点，进入下面的代码片段
		if (!IsRoot(y))
		{
			// 如果被删除节点是父节点的左子树，重设左子树，将右子树的值赋到被删除元素的位置，即被删元素的父节点的左子树,否则；相反
			if (IsLeftChild(y))
				SetLeftChild( Parent(y), x );
			else
				SetRightChild( Parent(y), x );
		}
		// 如果被删除元素是根节点，那么重置根节点为x
		else
			m_Root = x;
		
		// need to store this off now, we'll be resetting y's color
		NodeColor_t ycolor = Color(y);
		// note by lk 2012-02-01 如果y表示的不是要删除元素的节点索引,进入下面的代码片段
		if (y != elem)
		{
			// Standard implementations copy the data around, we cannot here.
			// Hook in y to link to the same stuff elem used to.
			// note by lk 2012-02-01 设置y索引的节点的父节点、左子树、右子树为被删除元素的父节点、左子树和右子树
			SetParent( y, Parent(elem) );
			SetRightChild( y, RightChild(elem) );
			SetLeftChild( y, LeftChild(elem) );
			
			// 如果被删除元素不是根节点，进入下面的片段
			if (!IsRoot(elem))
				if (IsLeftChild(elem))
				...
				if ((LeftChild(y) != InvalidIndex()) && (LeftChild(y) < m_TotalElements))
					SetParent( LeftChild(y), y );
				if ((RightChild(y) != InvalidIndex()) && (RightChild(y) < m_TotalElements))
					SetParent( RightChild(y), y );
				
				SetColor( y, Color(elem) );
		}
		// note by lk 2012-02-01 只有x元素为右子树时才执行RemoveRebalance函数
		if ((x != InvalidIndex()) && (x < m_TotalElements) && (ycolor == BLACK))
			RemoveRebalance(x);
	}
}
...
{
	// modify by lk 2012-02-01 增加检测条件
	if ((elem != InvalidIndex()) && (elem < m_TotalElements))
	{
		Unlink( elem );
		FreeNode(elem);
...
{
	I node = Find( search );
	if ((node != InvalidIndex()) && (node < m_TotalElements))
	{
		RemoveAt(node);
		return true;
	}
...
void CUtlRBTree<T, I>::FindInsertionPosition( T const &insert, I &parent, bool &leftchild )
{
	...
	// note by lk 2012-02-01 第一次插入元素时，m_Root始终为无效的索引值
	I current = m_Root;
	// note by lk 2012-01-31 InvalidIndex()函数返回一个模板整型的最大上限值，以unsigned short为例，该值为65535
	parent = InvalidIndex();
	leftchild = false;
	// note by lk 2012-02-01 下面的代码片段只有内存中存在元素时，才会进入
	while ((current != InvalidIndex()) && (current < m_TotalElements))
	{
		parent = current;
...
I CUtlRBTree<T, I>::Insert( T const &insert )
{
	...
	// note by lk 2012-02-01 I必须为正整型，默认为unsigned short
	I parent;
	// 左子树
	bool leftchild;
	// 搜索可以能够插入的位置
	FindInsertionPosition( insert, parent, leftchild );
	// note by lk 2012-02-01 通过FindInsertionPosition函数，parent和leftchild会被初始化,parent为元素插入的位置,leftchild决定插入的元素是左子树还是右子树
	I newNode = InsertAt( parent, leftchild );
	// note by lk 2012-02-01 拷贝构造函数，将新元素的内容拷贝到newNode表示的那块内存上
