...
//
// $Header: /cvs/myengine/src_main/public/valve/interface.cpp,v 1.1 2008/02/29 00:05:44 easap\albertyang Exp $
//
...
void* GetModuleHandle(const char* name)
{
	...
	if ((handle = dlopen(name, RTLD_NOW|RTLD_DEEPBIND)) == NULL)
	{
		//printf("Error:%s\n",dlerror());
...
CSysModule* Sys_LoadModule(const char* pModuleName)
{
	...
	Msg("Loading Moudle : %s\n", pModuleName);

	HMODULE hDLL = dlopen(pModuleName, RTLD_NOW|RTLD_DEEPBIND);
#endif
	...
	if (!hDLL)
	{
		...
		if (!hDLL)
		{
			Msg("load model error : %s\n", dlerror());
		}

		_snprintf(str, sizeof(str), "%s_486.so", szAbsoluteModuleName);
		hDLL = dlopen(str, RTLD_NOW|RTLD_DEEPBIND);
#endif
