...
public:
	...
	void PushPointer(void *ptr) { m_pAddressList.push_back(ptr); }
	void RemovePointer(void *ptr) { m_pAddressList.remove(ptr); }
...
// Release versions
//-----------------------------------------------------------------------------
static char const *pUnknown = "unknown_memdbg.cpp";
...
void *CDbgMemAlloc::Alloc( size_t nSize )
{
	...
	*/
	...
	void *pMem = Alloc( nSize, pUnknown, 0 );
...
{
	// FIXME: Should these gather stats?
	pMem = Realloc( pMem, nSize, pUnknown, 0 );
...
{
	m_OutputFunc("%s, line %i\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%d\t%d\t%d\t%d",
		pAllocationName,
		line,
...
void CDbgMemAlloc::DumpFileStats()
{
	...
	for (std::list<void*>::iterator iter = m_pAddressList.begin(); iter != m_pAddressList.end(); iter ++)
	{
		m_OutputFunc("leak address : %x, %d\n", *iter, GetSize(*iter));
	}
}
