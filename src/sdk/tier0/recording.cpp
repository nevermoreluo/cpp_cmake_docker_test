...
//
// $Header: /cvs/myengine/src_main/tier0/recording.cpp,v 1.1 2008/02/28 14:32:38 easap\albertyang Exp $
// $NoKeywords: $
//
...
static FILE* OpenRecordingFile()
{
	...
	if (!g_CantOpenFile)
	{
		...
		if( g_NeverOpened )
		{
			fp = fopen( "my_debug.log", "wbc" );
		}
#else
		fp = fopen( "my_debug.log", g_NeverOpened ? "wb" : "ab" );
#endif
		if (!fp)
		{
...
void RecordArgumentEx(const char* str, ...)
{
    ...
    va_start(args, str);
#ifdef _WIN32
    _vsnprintf( strBuffer, sizeof(strBuffer), str, args );
#else
    vsnprintf( strBuffer, sizeof(strBuffer), str, args );
#endif	
    va_end(args);

	RecordArgument(strBuffer, strlen(strBuffer));
}
